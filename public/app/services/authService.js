 ...
 .factory('Auth', function($http, $q, AuthToken,$cacheFactory) {
   ...
   authFactory.login = function(username,password) {
   		//return a promise object and it's data
   ...
   authFactory.logout = function() {
         ...
         httpCache.remove('/api/me'); 
   };
 
   // check if a user is logged in
   // check if their is a local token
   ...
   authFactory.getUser = function() {
   		if(AuthToken.getToken())
   			return $http.get('/api/me',{cache:true});  //caching the user info so we call the api only if needed
   		else
